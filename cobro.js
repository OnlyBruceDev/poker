document.addEventListener("DOMContentLoaded", () => {
  ...
  // Estado colapsado de cada tarjeta
  ...
  function deleteLibreClient(clientKey) {
    ...
    delete cardCollapseState[clientKey];
  }
  ...
  function buildClientCard(key, client, index, options) {
    ...
    const collapsed = cardCollapseState[key] !== false;
    card.className = "client-card" + (collapsed ? " collapsed" : "");
    ...
    header.addEventListener("click", e => {
      if (!["BUTTON", "I"].includes(e.target.tagName)) {
        ...
        cardCollapseState[key] = card.classList.contains("collapsed");
      }
    });
  ...
  function renderClients(clients) {
    ...
    filtered.forEach(entry => {
      const city = (entry[1].city || "Sin Ciudad").trim() || "Sin Ciudad";
      if (!groups[city]) groups[city] = [];
      groups[city].push(entry);
    });

    let idx = 0;
    Object.keys(groups).sort().forEach(city => {
      const groupWrap = document.createElement("div");
      groupWrap.className = "city-group";
      const head = document.createElement("h3");
      head.className = "city-header";
      head.textContent = city;
      groupWrap.appendChild(head);
      const cityList = document.createElement("div");
      cityList.className = "client-cards";
      groups[city].forEach(([key, client]) => {
        const card = buildClientCard(key, client, idx++, { showNotes: true });
        cityList.appendChild(card);
      });
      groupWrap.appendChild(cityList);
      list.appendChild(groupWrap);
    });

    const filteredObj = filtered.reduce((o,[k,v])=>{o[k]=v;return o;},{});
    totalRevenueEl.textContent = `Total Recaudado: $${calculateTotalRevenue(filteredObj).toLocaleString()}`;
  }
